...
3. 产出**两件套**：
   ...
   - 实操 `<subject_path>/lab/NN-主题…`（编号与课件对齐）——**载体由总控与学生确定**（总控写入 `lab/README.md`；你补上环境与依赖、验证方式）：notebook（`templates/lab-notebook.ipynb` 骨架）/ 源码+测试目录 / 页面 / 练习页 等。你按 `lesson-design` 第三节实现
