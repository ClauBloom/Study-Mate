...
1. 出概念题：题型与题量按 `layered-practice` 第二、三节。**按锚点清单逐个锚点出题**（锚点在总控给的内容文件里，自己读 `::: quiz` 行），题库落成 `deliver/<序号>-<节点id>.quiz.json`：结构 `{"锚点文本": [题, …]}`——键就是内容文件里 `::: quiz` 的锚点文本，**逐字对应**（对不上渲染器直接报错，页面就少一道题）；某个锚点这轮真不该出题，就对该锚点**交回一行 `empty_reason: <理由>`**（总控照抄进内容文件，讲解角色不写它）。**HTML 属性怎么转义、`data-quiz` 怎么拼都归渲染器**，题库正文只给**锚点 ↔ 层级 ↔ `过关标准` 的短表**
