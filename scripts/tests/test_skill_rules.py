...
'learning-system': [
 ...
 ('学生说今天到这别再推', '今天到这'),
 ('节点状态置学习中', '学习中'),
 ('值映射表逐角色只列值（评估角色行）', '`practice-evaluator` | `subject_path` + 节点 id + 「课件内容文件」绝对路径 + 载体（**时机二·评估**另加**作答原文**）'),
 ...
 ('新科目并行：资源清单落位后同时派两支', '**建池与拟大纲并行**——「资源清单」落位后，同时派下面两个（别串着等）'),
 ('并行两支各自的输入与汇合点一条链', '├─ 「采图」（爬图片库）             ← 值 = 「资源清单」的绝对路径 + 点名要抓的条目\n        └─ 「课设」（设计大纲）            ← 值 = 盘问结果摘要 + 「资源清单」路径\n     两边都回来 → 你写「课程大纲」、核对「图片库索引」'),
 ('图片库为空不阻塞', '图片库为空不阻塞'),
...
'record-keeping': [
 ...
 ('档案只留结构化摘要', '原始过程留在会话里'),
 ('lab 与说明页的归属在边界里也点名', '题目、lab 与实验说明页由「出题评估」落 `deliver/`'),
...
'image-scout': [
 ...
 ('边界：你只搬现成图（生成走另一条路）', '生成走另一条路（「讲解」自己跑库样例/画图表'),
