...
'learning-system': [
 ...
 ('开场默认先写暂存区，不探测工作区', '**默认先写暂存工作区，别去写 `LEARN_WORKSPACE`、也别做任何探测写入**'),
 ('唯一不暂存的情况：会话目录就是工作区', '**只有一种情况不暂存**'),
 ('暂存模式：暂存根落会话目录，不用 /tmp', '`<WS>` = `<TMP_WS>` = `<SESSION_DIR>/.studymate-stage/<slug>`'),
 ('暂存模式明令禁止拿 /tmp 当中转', '别拿 `/tmp` 当中转站'),
 ('暂存模式的两条硬约束', '**暂存模式的两条硬约束**'),
 ('跑引擎脚本加 -B（别往只读引擎目录写 __pycache__）', '跑引擎的 Python 脚本一律加 `-B`'),
 ('暂存模式开场告知学生', '课程先建在会话目录下的临时工作区'),
 ('暂存模式要求跨工具接力只在暂存根之下', '**每次落盘的下一步要能看见它**'),
 ('暂存模式收尾：问落点并搬走', '问一次成品放哪'),
 ('落点候选：学习工作区／桌面／文档／家目录', '**学习工作区**（`LEARN_WORKSPACE`，平时都在那，放进去就能续学）／**桌面**／**文档文件夹**／**用户根目录**'),
 ...
 ('落到工作区时刷新根主页', "gen_home.py '<LEARN_WORKSPACE>'"),
 ('暂存模式下不默默改配置', '**不要默默去改**'),
...
'record-keeping': [
 ...
 ('科目隔离', '科目之间隔离'),
 ('只写工作区', '绝不写会话目录'),
 ('工作区根由开场探测决定（可写=LEARN_WORKSPACE，否则暂存区）', '路径都以**工作区根 `<WS>`** 为前缀'),
 ('暂存模式是默认走法（跨工具接力、.stage 清空、搬走）', '暂存模式（默认走法；`<WS>` = `<SESSION_DIR>/.studymate-stage/<slug>`）的额外规则'),
 ('暂存模式禁止拿 /tmp 当中转（两个命名空间）', '别拿 `/tmp` 当中转'),
 ('角色产出走科目内的 .stage/', '角色产出走 `<subject_path>/.stage/`'),
