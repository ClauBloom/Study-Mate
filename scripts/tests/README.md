...
| `test_lesson_links.py` | 检查项 10（本地引用可达）：页面里的 href/src 必须落到真实文件。正文链接指错文件、科目组件或共享层缺文件都拦；外链、锚点、协议相对 `//`、HTML 注释里的路径、上下节课指针（落空是设计内）一律跳过，`?查询串` 与 `#片段` 先剥掉再解析；另有两条钉条件引用——页面里有公式就必须带离线 KaTeX 三件；还有一条钉生成产物——科目主页（gen_home 的产物）没生成时只提示不阻断（13 例） |
