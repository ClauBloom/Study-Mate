...
| `test_pool.py` | 图片库校验器 `check_pool.py`：表头七列齐全（分隔行跳过）、每行的图真在 `assets/img/pool/` 下、文件名合规（字符集 + ≤60 字符）、`来源 URL`/`许可`/`抓取日期` 非空、单张 ≤200 KB（按文件字节，不读 `尺寸` 列）（6 例） |
...
| `test_skill_rules.py` | 提示词回归：`.dsh/skills/*/SKILL.md` 里 401 条可执行规则逐条在位（压缩/改写时不许丢规则；`evidence-check` 的「原字段名 `evidence`」那条随提示词精简一并移除，映射仍留在 `docs/使用说明.md`） |
