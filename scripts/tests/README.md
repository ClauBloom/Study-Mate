...
| `test_render_lesson.py` | 课件渲染器 `render_lesson.py`：壳与接线齐全、正文与代码块的 `&<>` 转义且代码原文逐字、表格/列表/围栏/行内标记、题目按锚点合入且 `data-quiz` 单引号包裹与实体正确、锚点无题必须 `empty_reason`、配图存在性与题注来源、导航序号按大纲算、未知指令与手写 HTML 带行号报错、`--check` 不写盘、渲染产物过闸门（14 例） |
