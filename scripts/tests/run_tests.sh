...
step '本地引用可达（检查，14 例）'         python3 scripts/tests/test_lesson_links.py
...
step '课件渲染（渲染器，29 例）'           python3 scripts/tests/test_render_lesson.py
...
if command -v node >/dev/null 2>&1; then
  step 'quiz.js 渲染（40 项）'             node scripts/tests/quiz_dom_test.js
...
if [ "$BROWSER" = 1 ]; then
  if command -v google-chrome >/dev/null 2>&1; then
    ...
    step '公式排版（真实 Chrome，10 项）'     node scripts/tests/browser/math_test.mjs
  else
