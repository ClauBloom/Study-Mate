...
step '本地引用可达（检查，12 例）'         python3 scripts/tests/test_lesson_links.py
...
step '课件渲染（渲染器，28 例）'           python3 scripts/tests/test_render_lesson.py
...
if [ "$BROWSER" = 1 ]; then
  if command -v google-chrome >/dev/null 2>&1; then
    ...
    step '公式排版（真实 Chrome，6 项）'      node scripts/tests/browser/math_test.mjs
  else
