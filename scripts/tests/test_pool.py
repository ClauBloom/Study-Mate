...
`<subject>/assets/img/pool.md`、表头七列逐字固定。每个场景自造一个临时科目，只改一处偏差，
断言「该拦的拦住、该放行的放行」——校验器是"命名必须能检索"的保险，坏池子必须过不去。
...
# 形状合规但太长（69 字符）：字符集、分段、`<NN>` 都对，只有长度超——必须按长度拦
...
# 5 许可空：`许可` 那格是空的（页面没标注也要写「未标注」）
scenario('许可空拦',
         index(row(IMG_A, license_='')),
         {IMG_A: 400}, True, must=['`许可` 为空'],
         must_not=['文件名不合规', '文件缺失', '超体积'])

# 6 超体积：301 KB（`尺寸` 写 100×100 也救不了——体积只看文件字节）
scenario('超体积拦',
         index(row(IMG_B, size='100×100')),
         {IMG_B: 301 * 1024}, True, must=['超体积', '301 KB'],
